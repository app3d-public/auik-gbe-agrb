...
struct TexturedVertexBatchData
{
    vec2 offset;
    uint texture_id;
    uint flags;
...
layout(set = 0, binding = 1) readonly buffer Batches
{
    TexturedVertexBatchData batches[];
};
...
void main()
{
    ...
    TexturedVertexBatchData batch = batches[batch_id];
    vec2 pixel_pos = in_pos_z.xy + batch.offset;
    ...
    out_texture_id = batch.texture_id;
    out_flags = batch.flags;
}
